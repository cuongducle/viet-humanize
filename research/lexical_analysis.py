...
- fightin' words: log-odrs z-score (Monroe et al. 2008) từng âm tiết
...
"""
import glob, math, re, sys, unicodedata
...
hay hoặc vì nào nào cả lại đều vừa mới hơn nhất rất khá chỉ còn ngay các mỗi vài
từng cùng khi nào trong ngoài giữa đến tới theo i ê hoặc__ collaborator""".split())
FUNCTION.discard("collaborator"); FUNCTION.discard("hoặc__"); FUNCTION.discard("i"); FUNCTION.discard("ê")

INTENSIFIERS = ["rất", "khá", "vô cùng", "cực kỳ", "đặc biệt", "hoàn toàn", "chủ yếu",
                "tuyệt đối", "rõ ràng", "vô cùng", "hết sức", "cực", "siêu"]
...
def toks(path):
    ...
    t = re.sub(r"<[^>]+>", " ", t)
    words = re.findall(r"[0-9a-zA-ZàáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđĐ]+", t.lower())
    return words
